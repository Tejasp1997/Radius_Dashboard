      ...
      try {
       ...
       setOverallCount(data.overallCount);
        ...
        .filter((macAddress) => {
          ...
          const lastConnectionTime = new Date(user.latestLog.split(' ').slice(1, 6).join(' ')); // Assuming the timestamp format is consistent
       ...
       .map((user, index) => ({ ...user, macAddress: Object.keys(updatedConnectedUsers)[index] }))
      ...
      <div className="dashboard-content">
        ...
        <div className="logs-table">
          ...
          <table>
            ...
            <tbody>
              {/* {Object.keys(connectedUsers).map((macAddress) => {
                const user = connectedUsers[macAddress];
